...
_batch_consolidation_policy = (
    "Batch consolidation policy (workspace-mcp phase 8.1):\n"
    "- Before the FIRST mutation in your reply on a given target object \n"
    "  (spreadsheet / document / presentation / form / mailbox / contact book), \n"
    "  enumerate ALL planned operations on that object. If you find >=2 \n"
    "  same-type operations OR >=3 mixed-type operations on the same object, \n"
    "  you MUST consolidate into a single batch call. The same rule applies \n"
    "  to READS: do not loop over a list of ids with one-by-one fetches when \n"
    "  a batch reader exists. Sequential identical tool_calls when a batch \n"
    "  wrapper exists are a policy violation, regardless of intent.\n"
    ...
    "  * READS: do NOT make multiple read_sheet_values calls in a row on the \n"
    "    same spreadsheet. If you need structure (size, merges, frozen, sample \n"
    "    rows) — call read_sheet_summary ONCE (2-3 API calls regardless of size). \n"
    "    If you need values from several ranges — plan one widest read that \n"
    "    covers them, not three narrow reads. spreadsheets.values.batchGet is \n"
    "    not exposed as a tool yet; minimise read calls upstream by planning. \n"
    "    Three read_sheet_values calls in a row on the same spreadsheet is a \n"
    "    policy violation.\n"
    ...
    "  * Sending or drafting >=2 messages: there is NO batch sender. Each \n"
    "    send_gmail_message / draft_gmail_message is a separate user-visible \n"
    "    action. Before looping, ask the user to confirm the LIST of recipients \n"
    "    and the message body(ies) in chat, then proceed with the loop only \n"
    "    after explicit approval. Do NOT silently iterate over a recipient list.\n"
    "- Contacts:\n"
    "  * >=2 create/update/delete -> manage_contacts_batch, not a loop of \n"
    "    manage_contact.\n"
    "  * Looking up several people: prefer ONE search_contacts call with a \n"
    "    broader query, or one list_contacts page, over N get_contact calls \n"
    "    by resource_name. Use get_contact only when you already have the \n"
    "    exact resource_name and need full detail for one entry.\n"
    "- Drive: there is no batch wrapper. Sequential calls are fine when each \n"
    "  targets a different file. But: do not list_drive_items repeatedly with \n"
    "  narrowing filters — issue ONE query with the correct filter from the start. \n"
    "  For permission changes on the SAME file, prefer manage_drive_access (one \n"
    "  call covers add/update/remove/list) over multiple low-level calls.\n"
    "- Calendar: there is no batch wrapper for events. Sequential manage_event \n"
    "  calls for several events are unavoidable. Reads: query_freebusy already \n"
    "  accepts a list of calendars in one call — use it, do not loop get_events \n"
    "  across calendars just to compute availability.\n"
    "- Chat / Tasks: per-message and per-task tools have no batch. Loops are \n"
    "  the only option; minimise count by filtering upstream (search_messages \n"
    "  with the right time range, list_tasks with showCompleted/dueMin).\n"
    ...
    "- NEGATIVE EXAMPLE (DO NOT DO THIS): three read_sheet_values calls in a \n"
    "  row to A1:E5, then K1:M20, then A50:C60 on the same spreadsheet. \n"
    "  CORRECT: one read_sheet_summary if you only need shape and a sample, \n"
    "  or one widest read_sheet_values covering the needed area, parsed locally.\n"
    "- NEGATIVE EXAMPLE (DO NOT DO THIS): loop of get_gmail_message_content \n"
    "  for each id returned by search_gmail_messages. \n"
    "  CORRECT: pass the full id list to get_gmail_messages_content_batch.\n"
